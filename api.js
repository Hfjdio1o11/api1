...
// Ð?c danh sách key b? c?m t? t?p van b?n (n?u có)
...
// Ð?c danh sách allowed keys t? t?p van b?n
  ...
  try {
    ...
    if (!allowedKeys.includes(userKey)) {
      ...
      if (blacklistedUsers[clientIPv6].violations >= maxViolations) {
        ...
        // Ghi danh sách key b? c?m vào t?p van b?n
    ...
    if (method === 'raw') {
      ...
      const ls = spawn('node', ['raw.js', host, time, 64, 3, 'proxy.txt', 'hoandz']);
    } else if (method === 'https-browser') {
      // Ðua ra code cho method https-browser v?i các bi?n dã du?c d?nh nghia tru?c dó
      // ...
    } else {
